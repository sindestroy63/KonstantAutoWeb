 ...
 * Сохраняет исходник в reference/car-source-images/<slug>.<ext> и обновляет image в JSON.
...
const USER_AGENT =
  "KonstantAuto-ImageDownloader/1.0 (https://xn--80aag3apbefwkcc.xn--p1ai)";
...
function shouldSkip(car: CarRecord, projectRoot: string): boolean {
  const optimizedName = `${car.slug.replace(/_/g, "-")}.webp`;
  return fs.existsSync(path.join(projectRoot, "public", "images", "catalog", optimizedName));
}
...
async function main(): Promise<void> {
  ...
  const carsDir = path.join(projectRoot, "reference", "car-source-images");
  ...
  for (let i = 0; i < cars.length; i++) {
    ...
    const outPath = path.join(carsDir, `${car.slug}.${ext}`);
    ...
    car.image = `/images/catalog/${car.slug.replace(/_/g, "-")}.webp`;
    downloaded++;
    await sleep(randomDelay());
  }
