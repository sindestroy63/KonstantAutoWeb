...
export function HeroCarCarousel({ cars }: HeroCarCarouselProps) {
  ...
  return (
    <div
      className="group shine-overlay dark-card relative min-h-[420px] overflow-hidden rounded-[34px] p-4 sm:p-5"
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
      onFocus={() => setIsPaused(true)}
      onBlur={() => setIsPaused(false)}
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(255,76,76,0.12),transparent_34%),radial-gradient(circle_at_bottom_right,rgba(255,255,255,0.06),transparent_30%)]" />

      <div className="relative">
        <div
          key={currentIndex}
          className="relative h-[350px] overflow-hidden rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.02))]"
          style={{
        ...
        >
          <Link href={`/catalog/${car.slug}`} className="absolute inset-0 flex items-center justify-center p-5 sm:p-7">
            <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(4,8,14,0.02),rgba(4,8,14,0.4))]" />
          ...
          <div className="absolute inset-x-4 bottom-4 rounded-[24px] border border-white/10 bg-slate-950/[0.62] p-5 backdrop-blur-xl">
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                ...
                <p className="text-sm font-medium text-red-200">{benefitText}</p>
              </div>
      ...
      {total > 1 ? (
        <>
          <button
            ...
            className="absolute left-4 top-1/2 z-20 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/10 bg-slate-950/[0.52] text-white backdrop-blur-xl transition-colors hover:bg-slate-900/[0.76]"
          ...
          <button
            ...
            className="absolute right-4 top-1/2 z-20 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/10 bg-slate-950/[0.52] text-white backdrop-blur-xl transition-colors hover:bg-slate-900/[0.76]"
